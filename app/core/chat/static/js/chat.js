...
const userDiv = (senderId, receiverId, name, online) =>
    (`<a href="/posts/chat/${senderId}/${receiverId}" id="user${receiverId}" class="collection-item row">
                    <img src="https://cdn4.iconfinder.com/data/icons/glyphs/24/icons_user2-512.png" class="col s2">
                    <div class="col s10">
                    <span class="title" style="font-weight: bolder">${name}</span>
                    <span style="color: ${online ? 'green' : 'red'}; float: right">${online ? 'online' : ''}</span>
                    </div>
                </a>`)
...
function send(sender, receiver, message) {
    $.post('/posts/api/messages', '{"sender": "' + sender + '", "receiver": "' + receiver + '","message": "' + message + '" }', function (data) {
        console.log(data);
...
function receive() {
    $.get('/posts/api/messages/' + sender_id + '/' + receiver_id, function (data) {
        console.log(data);
        if (data.length !== 0) {
...
function getUsers(senderId, callback) {
    return $.get('/posts/api/users', function (data) {
        console.log(data);
...
function register(username, password) {
    $.post('/posts/api/users', '{"username": "' + username + '", "password": "' + password + '"}',
        function (data) {
            console.log(data);
            window.location = '/';
